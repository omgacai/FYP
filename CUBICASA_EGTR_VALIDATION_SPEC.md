...
`slurm/qwen3vl_prompt_ablation.sbatch` is the end-to-end smoke-test job: it builds a limited CubiCasa image/SVG manifest, derives fresh CubiGraph silver JSON and relation SVGs, then produces three separate conditions: direct image-only baseline, direct CubiCasa few-shot prediction, and fixed-node silver correction. The correction arm supplies Qwen with the original CubiGraph room IDs plus source-SVG-derived normalised boxes and candidate edges. Qwen returns an additive patch (`room_type_updates`, `edge_decisions`) only; a deterministic adapter applies it to the immutable source geometry and exports both the canonical graph and compact CubiGraph adjacency JSON. Every condition emits raw JSONL, silver metrics, and an HTML review bundle. The correction arm is still a VLM proposal, never an overwrite of the silver graph.

The correction patch additionally permits a tightly bounded `room_splits` proposal when one source room visibly contains two to four distinct rooms. A split replaces just that source ID with new boxed child IDs and drops the parent’s inherited edges; Qwen must propose any replacement topology separately. It is a high-risk disagreement signal for human review, not an automatic alteration of source SVG geometry or gold data.

All Qwen payloads are first parsed and strictly validated with Pydantic (including forbidden extra fields, required arrays, and confidence ranges), then checked for graph-specific duplicate/conflicting pairs. A malformed response triggers at most one text-only JSON formatter retry by default; the original answer, repair answer(s), validation error, and repair count remain in the JSONL for audit. A repaired answer is still a VLM proposal, not a gold label.
