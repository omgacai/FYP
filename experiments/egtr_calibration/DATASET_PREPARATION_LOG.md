...
| Edge-free graph target | Some valid room plans have no positive CubiGraph silver edge. EGTR's upstream relation sampler computes a mean over an empty sample and returns `NaN`. | The trainer preserves their room-detection supervision and assigns zero relation/uncertainty loss only for that plan. It does not invent a negative graph label. | `experiments/egtr_calibration/train_finetune.py` |
